...
const PATH_TO_TOOL_MAP: Record<string, ToolId> = {
  ...
  '/json-to-typescript': 'jsontocode',
  '/json-to-types': 'jsontocode',
};
...
// Helper to resolve route directly from window location without initial re-render jump
function resolveInitialRoute(): { tool: ToolId; topic: string } {
  if (typeof window === 'undefined') return { tool: 'home', topic: 'json' };
  ...
  if (searchPath.includes('/learn-')) {
    ...
    return { tool: 'education', topic };
  }

  if (searchPath === '/ads.txt') return { tool: 'ads_txt', topic: 'json' };
  if (searchPath === '/robots.txt') return { tool: 'robots_txt', topic: 'json' };
  if (searchPath === '/sitemap.xml') return { tool: 'sitemap_xml', topic: 'json' };
  if (searchPath === '/613e87bbb04e45cfa9f488bc6056e990.txt' || searchPath === '/613e87bbb04e45cfa9f488bc6056e990') {
    return { tool: 'indexnow_key', topic: 'json' };
  }
  ...
  return { tool: validTool || 'home', topic: 'json' };
}

export default function App() {
  const [initialRoute] = useState(() => resolveInitialRoute());
  const [activeTool, setActiveTool] = useState<ToolId>(initialRoute.tool);
  const [educationTopic, setEducationTopic] = useState<string>(initialRoute.topic);
  ...
  const navigateToTool = (id: ToolId, source: 'normal' | 'favorite' | 'recent' = 'normal') => {
    ...
    if (window.location.pathname !== fullPath && window.location.pathname.replace(/\/$/, '') !== fullPath.replace(/\/$/, '')) {
